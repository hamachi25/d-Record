...
async function changeUploadToggle(setShadow: (value: boolean) => void, workId: string | null) {
	setShadow(true);
	...
	if (uploadIcon() === "upload") {
		...
		cleanupIntervalOrEvent(); // インターバル・イベントを削除
		...
		createIntervalOrEvent(); // インターバル・イベントを作成
...
export default function UploadToggleButton() {
	const [shadow, setShadow] = createSignal(false); // クリック時の影
	...
	return (
		<>
			...
			<div id="upload-icon-container" onClick={() => changeUploadToggle(setShadow, workId)}>
				<img
					id="upload-icon"
					src={updateIcon()}
					width={28}
